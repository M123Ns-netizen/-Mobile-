...
<main>
  <section class="card">
    ...
    <!-- البحث المباشر مثل Excel -->
    <input id="nicknameInput" type="text" placeholder="اكتب للبحث..." oninput="liveSearch()">
...
<script>
  const secretCode = "#snow"; // الكود السري

  // تحميل القائمة من التخزين
  ...
  function renderList(filtered = null) {
    ...
    let data = filtered || nicknames;
    data.sort((a, b) => a.localeCompare(b, "ar"));
  ...
  function liveSearch() {
    ...
    // فلترة
    const filtered = nicknames.filter(n => n.toLowerCase().includes(text));

    renderList(filtered);

    if (filtered.length === 0) {
      status.textContent = "لا يوجد نتائج.";
